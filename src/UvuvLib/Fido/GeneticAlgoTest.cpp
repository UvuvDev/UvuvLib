...
net::NeuralNet GeneticAlgo::getBestNeuralNetwork(int numberOfGenerations, net::NeuralNet modelNetwork) {
  ...
  std::vector<double> testInputs = { 1, .3, .4, .1, 1, .3, .4, .1, 1, .3, .4, .1 };

  std::vector<double> testInputsTwo = { 1 };
  ...
  for(unsigned int a = 0; a < numberOfGenerations; a++) {
      ...
      modelNetwork.getOutput(testInputsTwo);
  ...
  int mostFitIndex = 0;
  for(unsigned int a = 1; a < fitnesses.size(); a++) {
      if (fitnesses[a] > fitnesses[mostFitIndex]) {
          mostFitIndex = a;
      }
  }
